...
# -*- coding: utf-8 -*-
"""
Created on Sun Sep 27 17:06:21 2022
last modefied on Mon Mar 27 09:16:30 2023
"""
...
def preprocess(texts):
    ### added
    os.sys.stdout.write("Start: preprocess()\n")
...
def tokenize(textA,textB,seq_len=102):
    ### added
    os.sys.stdout.write("Start: tokenize(), Sequence Length: %d\n"%(seq_len))
...
def train(args,model,train_a,train_b,tr_attn_a, tr_attn_b, label_tr):
    ...
    for i, (pair_a, pair_b, attna, attnb, y) in tqdm(enumerate(batchify(train_a,train_b,tr_attn_a, tr_attn_b, label_tr,size=args.training_bsz,shuffling=True))):
        ...
        ### Added
        print("type(pair_a):",type(pair_a))
        print("type(pair_b):",type(pair_b))
...
def evaluate(args,test_a,test_b,te_attn_a, te_attn_b, label_te):
    os.sys.stdout.write("Evaluation using function evaluate()\n")
    ...
    for i, (pair_a, pair_b, attna, attnb, y) in tqdm(enumerate(batchify(test_a,test_b,te_attn_a, te_attn_b, label_te,size=args.test_bsz))):
        ...
        ### pair_a, pair_b are the vectors after pre-processing
        ### hiddenA, hiddenB are outputs of Bert
        ### added
        pair_a_list += list(pair_a.detach().cpu().numpy())
        pair_b_list += list(pair_b.detach().cpu().numpy())
        
        with torch.no_grad():
            hiddenA = model(pair_a.to(device),att_mask=attna.to(device))
            hiddenB = model(pair_b.to(device),att_mask=attnb.to(device))
            ### added
            hiddenA_list += list(hiddenA.detach().cpu().numpy())
            hiddenB_list += list(hiddenB.detach().cpu().numpy())

            print("hiddenA:",hiddenA)
            print("type(hiddenA):",type(hiddenA))
            # print("list(hiddenA.cpu().numpy()):",list(hiddenA.cpu().numpy()))
            print("hiddenA.cpu().numpy().shape:",hiddenA.cpu().numpy().shape)
            
            os.sys.stdout.write("Evaluation distance: %s\n"%(args.distance))
            if args.distance == 'cosine':
                os.sys.stdout.write("Calculating cosine distance\n")
                hiddenA = F.normalize(hiddenA,dim=1)
                hiddenB = F.normalize(hiddenB,dim=1)
                logit = 1.0-torch.sum(hiddenA*hiddenB,dim=1)
            elif args.distance == 'euclidean':
                os.sys.stdout.write("Euclidean cosine distance\n")
                logit = torch.norm(hiddenA - hiddenB,dim=1)
            print("logit:",logit)
            print("type(logit):",type(logit))
            print("y:",y)
            print("type(y):",type(y))
            # print("list(y.cpu().numpy()):",list(y.cpu().numpy()))
            print("type(list(y.cpu().numpy())):",type(list(y.cpu().numpy())))
            print("len(list(y.cpu().numpy())):",len(list(y.cpu().numpy())))
            # stop
    ...
    return logits,targets,hiddenA_list,hiddenB_list,pair_a_list,pair_b_list
...
if __name__ == "__main__":
    ...
    parser.add_argument("--save_model",default='save_data',type=str)
    ...
    ### Added 
    parser.add_argument('--token_len',default=100,type=int)
    ...
    if args.loss == 'margin':
        ...
        threshold = 0.5*args.tau_low+0.5*args.tau_high        
    os.sys.stdout.write("threshold: %e\n"%(threshold))
    ...
    if not os.path.exists(os.path.join(args.save_model,model_name)):
        ...
        # to
        os.makedirs(os.path.join(args.save_model,model_name))
        
    ### added 
    os.sys.stdout.write("args.train: %s\n"%(args.train))
    if args.train:
        ### added 
        os.sys.stdout.write("args.save_cache: %s\n"%(args.save_cache))
        # load data
        if args.save_cache:
            ### added 
            os.sys.stdout.write("Load training and develop data\n")
            ...
            # to
            train_a, train_b, tr_attn_a, tr_attn_b = tokenize(trainA, trainB, seq_len=args.token_len+2)
            test_a, test_b, te_attn_a, te_attn_b = tokenize(testA, testB, seq_len=args.token_len+2)

            ### Added
            if not os.path.exists(os.path.join(args.save_cache)):
                os.makedirs(os.path.join(args.save_cache))
            if not os.path.exists(os.path.join(args.save_cache)):
                os.makedirs(os.path.join(args.save_cache))
            ...
            ### added
            os.sys.stdout.write("Load training and develop data from cache (%s)\n"%(args.load_cache))
        ...
        for k in range(args.epochs):
            ### added
            os.sys.stdout.write("Epoch: %d\n"%(k))
            with open(os.path.join(args.save_model,model_name,'results'),'a+') as out:
                ### Added
                print("type(model):",type(model))
                print("type(train_a):",type(train_a))
                print("type(train_b):",type(train_b))
                print("type(tr_attn_a):",type(tr_attn_a))
                print("type(tr_attn_b):",type(tr_attn_b))
                print("type(label_tr):",type(label_tr))
                # print("model.shape:",model.shape)
                print("train_a.shape:",train_a.shape)
                print("train_b.shape:",train_b.shape)
                print("tr_attn_a.shape:",tr_attn_a.shape)
                print("tr_attn_b.shape:",tr_attn_b.shape)
                print("len(label_tr):",len(label_tr))
                ...
                logits,targets,hiddenA_list,hiddenB_list,pair_a_list,pair_b_list = evaluate(args,test_a,test_b,te_attn_a, te_attn_b, label_te)
...
#        text_te, label_te = shuffle(text_te, label_te)
        if args.save_cache:
            ### added
            os.sys.stdout.write("Load test data, preprocess, and tokenize\n")
            testA, testB = preprocess(text_te)
            ### Changed
            # from
            # test_a, test_b, te_attn_a, te_attn_b = tokenize(testA, testB)
            ...
            ### added
            os.sys.stdout.write("Load test data from cache\n")
        ...
        logits,targets,hiddenA_list,hiddenB_list,pair_a_list,pair_b_list = evaluate(args,test_a,test_b,te_attn_a, te_attn_b, label_te)
        ### added
        # np.save(args.save_model+'/logits',logits)
        # np.save(args.save_model+'/targets',targets)
        # np.save(args.save_model+'/hiddenA',hiddenA_list)
        # np.save(args.save_model+'/hiddenB',hiddenB_list)
        # np.save(args.save_model+'/pair_a_list',pair_a_list)
        # np.save(args.save_model+'/pair_b_list',pair_b_list)
        np.save(os.path.join(args.save_model,model_name,'logits'),logits)
        np.save(os.path.join(args.save_model,model_name,'targets'),targets)
        np.save(os.path.join(args.save_model,model_name,'hiddenA'),hiddenA_list)
        np.save(os.path.join(args.save_model,model_name,'hiddenB'),hiddenB_list)
        np.save(os.path.join(args.save_model,model_name,'pair_a_list'),pair_a_list)
        np.save(os.path.join(args.save_model,model_name,'pair_b_list'),pair_b_list)
        ...
        if args.save_test_data:
            print("Save test data:",args.save_test_data)
            with open(os.path.join(args.save_model,model_name,'full_evaluation'),'w') as out:
                for u, p, logit, target in zip(users,products,logits,targets):
                    out.write("%s\t\t%s\t\t%s\t\t%s\n"%(u,p,logit,target))                                      
            with open(os.path.join(args.save_model,model_name,'full_evaluation_all'),'w') as out:
                for u, p, logit, target, hiddenA, hiddenB, pair_a, pair_b in zip(users,products,logits,targets,hiddenA_list, hiddenB_list, pair_a_list, pair_b_list):
                    out.write("%s\t\t%s\t\t%s\t\t%s\n"%(u,p,logit,target))
                    # print(u,p,logit,target,hiddenA.shape,hiddenB.shape,pair_a.shape,pair_b.shape)
